...
const screenSize = (dimensions?: 'screen' | 'window') => {
    return {
        width: Dimensions.get(dimensions ?? 'screen').width,
        height: Dimensions.get(dimensions ?? 'screen').height,
    } as Size;
};

const Provider = ({ children, zIndex }: { children: React.ReactNode | React.ReactNode[], zIndex?:number }) => {
    const data = {
        items: new Map(),
        update: () => { },
        find: (id: string) => {
...
const InternalModal = ({ item, index, zIndex }: { item: Item; index: number, zIndex:number }) => {
